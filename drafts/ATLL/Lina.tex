...
attack tree in one we are defining, then we can make use of
\mh{insert}.  Behind the scenes Lina maintains a special state that
tracks the identifiers of each node, thus, when one wishes to insert
an existing attack tree, which will have its own identifier labeling,
into a new tree, then that internal state must be updated, thus,
\mh{insert} carries out this updating.  Lina is designed so that the
user never has to encounter that internal state.
