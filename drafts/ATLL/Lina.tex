...
example, in Fig.~\ref{fig:vehicle_attack_decomp} we break up
\mh{vehicle_attack} into several smaller attack trees.
\begin{figure}  
  \begin{tabular}{|l|l|l|}
    \hline & \\[-8px]
    \begin{tabular}{l}
...
essentially the exact same specification as the SPTool, but with one
improvement discussed below, and the latter corresponds to the
quaternary semantics defined in
...
Fig.~\ref{fig:causal_attack_tree_maude_specification}.
\begin{figure}
\begin{mdframed}      
\scriptsize
\begin{verbatim}
...
\end{verbatim}
\end{mdframed}
  \caption{Maude Specification for Causal Attack Trees}
  \label{fig:causal_attack_tree_maude_specification}
\end{figure}
This specification has one improvement over the specification used by
the SPTool \cite{Kordy2017}.  Kordy et al.'s specification uses
strings to represent base attacks, but we use integer values.  Thus,
their specification is less general because it ties the attack trees
to a particular data type for labels, and arguably less efficient
because strings are harder to compare than integers.  As one can see
this specification only supports proving equivalences, but what about
specializations?

Lina also supports proving specializations between attack trees using
the MATLL Maude backend.  Its full Maude specification can be found in
Fig.~\ref{fig:maude-spec-matll}.
\begin{figure}
\begin{mdframed}
\scriptsize
\begin{verbatim}
...
Lina defines an attack as the following data type:
\begin{mdframed}
  \begin{tabular}{cccccccc}
  \\ 
  \begin{minipage}{2in}
...
\end{tabular}
\end{mdframed}
An attack does not have any choice nodes, because they are all split
into multiple attacks; one for each child node.  For example, the set
of possible attacks for the autonomous vehicle attack from
Fig.~\ref{fig:vehicle_attack} can be found in
