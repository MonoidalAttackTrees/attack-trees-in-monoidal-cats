...
-----------------------------------------------------------------------
module DialSets (L : Set) (l-pf : Lineale L) where
...
compatL : {a : L} {b : L} → rel (poset (mposet l-pf)) a b ≡ tt →
...
-- The tensor functor: ⊗
_⊗ᵣ_ : ∀{U X V Y : Set} → (U → X → L) → (V → Y → L) → ((U × V) → ((V → X) × (U → Y)) → L)
...
-- Internal hom:
⊸-cond : ∀{U V X Y : Set} → (U → X → L) → (V → Y → L) → (U → V) × (Y → X) → U × Y → L
